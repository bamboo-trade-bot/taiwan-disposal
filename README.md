...
現況清單會顯示每檔相對「**進處置前一個交易日收盤價**」的漲跌幅。

定義：

- 基準是該檔**目前這一段處置的最早起算日**，再往前一個交易日的收盤價
- 現價優先用即時報價，因此盤中會跟著跳動；沒有報價時退回最後收盤
- 數字可正可負（處置期間照樣可能上漲），沿用台股紅漲綠跌

> 基準取「整段的起點」而不是最後那張公告的起點，是因為像雙鴻這種前一次還沒
> 結束又接一次的個股，若從後面那張量起會嚴重低估跌幅。
...
一檔可能前一次處置還沒結束又接一次（雙鴻 09/03–09/09 第一次、09/08–09/14 第二次）。
這種情況在名稱旁標示 `×2`，該列已合併並取其中最嚴格的條件——最晚的解除日、最長的
撮合間隔、較嚴格的預收規定。
