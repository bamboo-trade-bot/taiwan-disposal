...
處置狀態（處置中／即將開始／今日出關／今日最後一天）一律以**開啟頁面當下的台北
日期**現算，不用抓取時寫死的日期。

這不是潔癖，是必要的：GitHub 的排程常被延後數小時（實測設 09:30 被排到 14:24、
設 18:30 被排到 22:31）。若沿用建置日期，早上打開會整頁停在昨天——但昨晚抓的公告
其實**已經含有今天的名單**，只是被判定成昨天的狀態。改成前端現算之後，每天午夜
自動就對了，不必等 CI。

因此 payload 會附上休市日清單，前端才能自行判斷交易日。
若公告資料超過 18 小時未更新，頁面會標示出來——當日新增的處置可能尚未納入。
...
|---|---|---|
| `17 0 * * 1-5` | 08:17 | 盤前刷新基準價與名單 |
| `17 9 * * 1-5` | 17:17 | 收盤後，當日處置公告多半已發布 |
| `17 12 * * 1-5` | 20:17 | 補抓當日較晚發布的公告 |

**一律避開整點與半點**：那是 GitHub 排程的尖峰，實測設 09:30 會被排到 14:24、
設 18:30 會被排到 22:31。用奇數分鐘可明顯減少排隊，但仍不保證準時——所以頁面
才會改用瀏覽器日期判定，不依賴排程準時。
...
|---|---|
| 即將開始 | 今天 < 起算日 |
| 處置中 | 起算日 ≤ 今天 ≤ 迄日（迄日當天另標「今日最後一天」）|
| **待出關** | 迄日 < 今天 < 出關日 |
| 今日出關 | 今天 = 出關日 |
| 已結束 | 今天 > 出關日 |

「待出關」是**處置期已結束、但還沒到第一個可正常交易日**。週五處置結束、週一才出關的話，
整個週末都落在這一段。沒有這個狀態的話那幾天該檔會從清單上整個消失——偏偏週末正是會想
先查「下個交易日誰恢復交易」的時候。基準價的抓取範圍也涵蓋這個狀態，否則那幾天會少一欄。

網址加 `?today=YYYY-MM-DD` 可指定日期檢視，用來查某天的狀態或驗證週末行為。
...
**兩個 API 的日期參數篩的都是「處置期間」，不是公布日期。** 這點很容易誤會，會導致漏抓：

| 查詢範圍（3055 於 09/11 公告，期間 09/14–09/18）| 查得到嗎 |
|---|---|
| 9/1 – 9/11 | 否 |
| 只查公布日 9/11 | 否 |
| 只查處置期間 9/14 – 9/18 | 是 |

所以抓取時**結束日一律往後延 30 天**，否則「已公告、但隔天以後才起算」的處置會整個被排除——
而這是常態：週五公告、週一起算的，會整個週末都看不到。

另外兩點：

- 跨月的處置會在相鄰兩個月都出現，抓完要依（公布日、代號、期間）去重。
- **上市**的長區間查詢會被伺服器**去重成每檔證券一列**（例：114/01/01–114/09/08 單次查詢只回 85 列，逐月加總則有 130 筆公告），所以一律以「月」為單位分段抓。
